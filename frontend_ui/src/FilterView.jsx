...
import { Card, CardContent, Container, Button, Grid } from '@mui/material';
...
export default function FilterView() {
  ...
  const handleFilter = async (e) => {
    ...
    selectedProjects.forEach(project => queryParams.append('project_id', project.id));
    selectedPathways.forEach(pathway => queryParams.append('pathway_id', parseInt(pathway.id)));
    selectedMilestones.forEach(milestone => queryParams.append('milestone_id', milestone.id));
    selectedActivities.forEach(activity => queryParams.append('activity_id', activity.id));
      ...
      <Card variant="outlined">
        <CardContent>
          <Grid container spacing={2}>
            ...
            <Grid item xs={6}>
            <MilestoneAutocomplete setSelectedMilestones={setSelectedMilestones} />
            </Grid>

            <Grid item xs={6}>
            <ActivityAutocomplete setSelectedActivities={setSelectedActivities} />
            </Grid>

            <Grid item xs={6}>
            <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker label="Start date" value={startDate} onChange={(v) => {
            ...
            <Grid item xs={6}>
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker label="End date" value={endDate} onChange={(v) => {
