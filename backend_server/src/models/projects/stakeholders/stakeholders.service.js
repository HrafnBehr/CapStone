...
async function getStakesbyid(id) {
    const stakes = await db('stakeholders')    
        .where({id})
        .first();
    return stakes;
}
...
async function CreateStake(stakes) {
    const [newStakes] = await db('stakeholders')
        .insert(stakes)
        .returning('*');
    return newStakes;
}
...
async function updatedStakes(id, stakes) {
    const Staketoupdate = await db('stakeholders')
        .where({ id })
        .first();
    
    const [updatedstakes] = await db('stakeholders').where({ id }).update({
        ...Staketoupdate,
        ...stakes
    }).returning('*');
...
module.exports = { getAllstakes, getStakesbyid, CreateStake, deleteStake,updatedStakes}