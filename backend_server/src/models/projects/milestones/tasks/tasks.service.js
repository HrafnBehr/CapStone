...
async function getAllTasks(milestone_id) {
  const tasks = await db('tasks').where({ milestone_id });
  return tasks;
}
...
async function createTask({ title, completed, start_date, due_date, project_id, milestone_id }) {
